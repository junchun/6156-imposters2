...
def make_pairs_10_fold(X, Y, pos_ratio = 1.0, neg_ratio=1.0, add_all_neg=False):
    ...
    positive_count = functools.reduce(lambda n1, n2: n1+n2, map(lambda num: num*num/2,
                                          y_dist.tolist()))
    X_left = []
    X_right = []
    trainY = []
    ...
    sample_X_left = np.array(X_left,dtype=object)[indices]
    sample_X_right = np.array(X_right,dtype=object)[indices]
