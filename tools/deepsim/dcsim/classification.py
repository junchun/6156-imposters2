...
def init_weights(shape, name):
    return tf.get_variable(name=name, shape=shape, dtype=tf.float32,
                           initializer=tensorflow.contrib.layers.variance_scaling_initializer(
...
def train_10_fold_balanced():
    ...
    for train_idx, test_idx in skf.split(X, y):
        ...
        with tf.Session() as sess:
            ...
            step = 0
            now = datetime.now()
            print("starting epoch at: ", now.strftime("%H:%M:%S"))
            for epoch in range(4):
                ...
                for start, end in zip(
                        ...
                        range(batch_size, np.shape(train_X_left)[0] + 1,
                              batch_size)):
                    now = datetime.now()
                    print('epoch %d, iteration %d start at: \n' % (epoch, step), now.strftime("%H:%M:%S"))
